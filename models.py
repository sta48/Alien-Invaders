...
class Ship(GImage):
    ...
    def __init__(self):
        super().__init__(x=400,y=SHIP_BOTTOM,width=SHIP_WIDTH,
                         height=SHIP_HEIGHT,source='ship.png')

    # METHODS TO MOVE THE SHIP AND CHECK FOR COLLISIONS
    def _move(self,x):
        if (self.x + x < GAME_WIDTH) and (self.x + x > 0):
            self.x = self.x + x
...
class Alien(GImage):
    ...
    LIST MORE ATTRIBUTES (AND THEIR INVARIANTS) HERE IF NECESSARY
    _alien1: one alien image
    _alien2: one alien image
    _alien3: one alien image
    """
